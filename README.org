...
variables and then =include=-ing =Makefile.common= from =mrbuild=. That file
produces the rules that =make= uses to do its job. Note that unlike the
CMake-based system, this is a one-step process: we don't create Makefiles and
then use them to build stuff; we just build the stuff.
